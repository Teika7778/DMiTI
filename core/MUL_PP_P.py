...
class MUL_PP_P(gm.AbstractModule):
    ...
    def execute(self, args):
        ...
        for degree in range(len(args[1]).coefficients): # Цикл по элементам второго многочлена
            temp_result = self.mul_pq_p.execute([args[0], args[1].coefficients[degree]])[0]  # промежуточный результат - результат умножения
            temp_result = self.mul_pxk_p.execute([temp_result,degree])[0] # всего P(x) на i-ый коэф Q(x) и умноженный на x^i
            result.add_pp_p.execute(result, temp_result) # Ответ - сумма таких промежуточных результатов
        return [result]
