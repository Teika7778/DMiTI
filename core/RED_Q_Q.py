...
class RED_Q_Q(gm.AbstractModule):
    ...
    def execute(self, args):
        ...
        numerator = self.div_zz_z.execute([rational.numerator, gcf])[0] # делим числитель на НОД
        denomirator = self.div_zz_z.execute([Integer(rational.denominator, is_positive = True), gcf])[0]# делим знамянатель на НОД
