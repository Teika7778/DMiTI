...
class DIV_PP_P_test(unittest.TestCase):
    ...
    def test_division_with_zero(self):
        coef1 = [
            Rational(Integer(Natural([2])), Natural([1])),  # 2
            Rational(Integer(Natural([1])), Natural([11]))   # 1/11
        ]
        p1 = Polynomial(coef1)   # 1/11x + 2
        coef2 = [
            Rational(Integer(Natural([2])), Natural([1])),  # 2
            Rational(Integer(Natural([1])), Natural([11])),  # 1/11
            Rational(Integer(Natural([1])), Natural([19])),  # 1/19
            Rational(Integer(Natural([69])), Natural([1])),  # 69
            Rational(Integer(Natural([52])), Natural([1]))  # 52
        ]
