...
class TestLED_P_Q(unittest.TestCase):
    ...
    def test_normal_polynomial(self):
        ...
        result = self.lead_coeff_finder.execute([poly])[0]
        expected = Rational(Integer(Natural([5])), Natural([1]))
        self.assertEqual(str(result), str(expected), "Expected leading coefficient to be 5")
    ...
    def test_polynomial_with_trailing_zeros(self):
        ...
        result = self.lead_coeff_finder.execute([poly])[0]
        expected = Rational(Integer(Natural([5])), Natural([1]))
        self.assertEqual(str(result), str(expected), "Expected leading coefficient to be 5, ignoring trailing zeros")
    ...
    def test_zero_polynomial(self):
        ...
        self.assertEqual(str(result), str(expected), "Expected leading coefficient to be 0 for zero polynomial")
